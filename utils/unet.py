...
class ResBlock(TimestepBlock):
    ...
    def __init__(
        ...
        self.out_norm = lambda x: F.group_norm(x.float(), 32, eps=1e-3).type(
            x.dtype)  # our arch didn't use an affine transform.
...
class UNetModel(nn.Module):
    ...
    def __init__(
        ...
        time_embed_dim = int(model_channels * channel_mult[
            -1])  # our imagenet model used a smaller time embedding dim because we used model splitting - we thought that smaller timestep ranges didn't need as many dims.
        ...
        ch = input_ch = int(channel_mult[0] * model_channels)  # 128
        # print('channel_mult[0]=', channel_mult[0], 'model_channels=', model_channels)
        # print('channel=', ch)
        # print('in channel=', in_channels)
        ...
        if self.learn_sigma:
            ...
            # )
            self.out.add_module('zero_module',
                                zero_module(conv_nd(dims, input_ch, in_channels, 3, padding=1))
                                )
        else:
            # self.out.append(
    ...
    def get_classifier_free_label(self, y):
        if not self.classifier_free:
            raise RuntimeError(
                "You are trying to get the unconditional label for a model that wasn't trained with classifier-free guidance.")
